import nltk #natural language processing pythonlibrary

# -------------------------------------------------------------------------
#                      HASHTAG RECOMMENDATIONS CLASS
# -------------------------------------------------------------------------
...
class HashtagRecs:
    ...
    """
    def __init__(self):
        self.hashtags = [] # empty list to store generated hashtags as instance attribute

    # This method uses nltk to analyse the user's post to extract keywords from it and return a string of the keywords.
    def extract_keywords(self, content):
        ...
        """
        # The word_tokenize function from NLTK splits the content into a list of words and punctuation.
        tokens = nltk.word_tokenize(content) 
        # The pos_tag function from nLTK then takes the tokenized content and adds a tag to each string tags is therefore
        ...
        keywords = [word for (word, tag) in tags if len(word) > 2 and (tag.startswith('NN') or tag == 'JJ' or tag == 'NNP')]
        return keywords
    
    # This method takes the keywords and puts them into the class's one instance atrribute, which is a list called hashtags.
